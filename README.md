...
| `duplicate_integrity_check_failed` | Expanding the repeat references did not reproduce the original exactly; the output was kept. |
| `duplicate_readability_check_failed` | Source preservation passed, but Jev did not approve the repeat notation’s readability. |
...
Retained text is copied from the source. Tool output can represent exact repeated blocks with source-line references while retaining every distinct block verbatim. The engine first expands those references and verifies byte-for-byte equality with the original, then asks Jev whether the repeat notation is understandable. Semantic deletion requires a combined preservation check. API failures, timeouts, uncertainty, and insufficient savings preserve the original. Older tool exchanges can be removed during Jev history compaction; standard Codex compaction remains the fallback. The visible chat transcript is not cleared.
...
Local checks include 38 Swift tests (including clipboard shortcuts, word coverage, literal preservation, rejection of meaning loss, and usage totals), five bridge protocol tests, a live helper/key-priority check, and 104 scoped Rust compaction regressions. Four live Strict message examples produced reductions; one went from approximately 24 to 8 tokens and a code-containing example from 31 to 15 with its code unchanged. The tool-output verifier has 11 passing targeted tests, including live focused-task and broad-task cases and rejection of ambiguous repeat notation. After correcting duplicate verification, a full app-server test accepted a repetitive tool result at approximately **7,700 → 967 tokens**, preserving `FINAL_STATUS=OK`. Exact source preservation is checked byte-for-byte before Jev judges readability; readability acceptance does not authorize losing content. A separate history-compaction exercise reached Jev but fell back to native compaction, so accepted history savings have not been established. A long outgoing courtesy fixture also retained its original after a failed preservation check. These are examples, not an accuracy or cost benchmark.
