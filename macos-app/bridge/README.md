...
Only a single plain text item with no text-element offsets is eligible. Multipart text, attachments, routing, models, reasoning effort, tool activity, approvals, and other RPC messages pass through. Each task keeps its request order; responses to server approval requests bypass the filter queue. API failures and a 85-second helper timeout retain the original message. Non-stdio CLI commands pass directly to the real engine.

The menu bar Settings message-compression toggle controls the bridge (`$CODEX_HOME/jev-message-settings.json`). Use **Jev → Settings…** to edit the key and preferences; this experiment does not add Jev controls to the official app's Settings. Original/prepared text archives remain under `jev-message-originals`. A metadata-only `jev-bridge/activity.jsonl` reports API attempts and estimated savings. It records preprocessing, not provider billing or proof of turn completion. The official app may display the shortened text when it receives the engine's turn data.
...
Outgoing mode is configurable in **Jev → Settings…**: **Balanced** (default) favors readable prose; **Strict (caveman)** aims for telegraphic text. Both send the complete eligible message and mechanically indexed word occurrences to Jev. Jev decides what is needed and what is code, quoted, or otherwise verbatim; code has no semantic parsing or word-selection heuristics. Batches contain at most 96 words, each with the full message. A separate Jev check verifies the combined candidate; Strict can retry a more conservative cutoff. Incomplete reviews and failed checks preserve the original. Both use the saved key first. The mode is persisted as `"mode":"strict"` or `"mode":"balanced"` beside `enabled` in `$CODEX_HOME/jev-message-settings.json`. This setting does not change history compaction.

Before/after estimated token counts are included in outgoing-message metadata. The menu app combines these with `jev-bridge/engine-activity.jsonl` for persistent counters; neither log contains message bodies or API keys. Engine updates take effect after relaunching the trial instance.
